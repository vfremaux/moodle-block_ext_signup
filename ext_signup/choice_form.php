...
class Course_Choice_Form extends moodleform{
    ...
    function definition(){
        ...
        if (!empty($CFG->block_ext_signup_courses)){
            $available_courses = get_records_select('course', " visible = 1 AND id IN({$CFG->block_ext_signup_courses}) ", 'fullname ASC', 'id, fullname, shortname, summary', $this->from, $this->page);
        } else {
