...
if ($action == 'deleteall'){
    ...
    $DB->delete_records_select('block_ext_signup', " userid = $userid AND accepted = ".EXT_PENDING);
    if ($DB->record_exists('user', array('id' => $userid, 'confirmed' => 0))){
...
elseif ($action == 'sendall'){
    ...
    if($inputs){
        ...
        foreach($inputs as $input){
            ...
            switch($resolve){
                case 'accept' :
                    if (!$exts = $DB->get_records_select('block_ext_signup', " userid = $userid AND courseid = $courseid AND accepted = ".EXT_PENDING)){
                    ...
                    $ext->reason = '';
                    $DB->update_record('block_ext_signup', $ext);
                ...
                case 'reject' :
                    ...
                    break;
                case 'delete' :
                    $DB->delete_records_select('block_ext_signup', " userid = $userid AND courseid = $courseid AND accepted = ".EXT_PENDING);
                    break;
            }            
        }
    }
    ...
    if (!empty($onewasaccepted)){
        ...
        $DB->set_field('user', 'confirmed', 1, array('id' => $ext->userid));
    ...
    if (!empty($processed)){
        ...
        foreach($processed as $extenroll){
            if ($extenroll->accepted == EXT_ACCEPTED){
                $coursename = $DB->get_field('course', 'fullname', array('id' => $extenroll->courseid));
                $vars['COURSE'] = $coursename;
                $coursecontext = context_course::instance($extenroll->courseid);
                $studentrole = $DB->get_record('role', array('shortname' => 'student'));
...
elseif ($action == 'rejectall'){
    ...
    $exts = $DB->get_records('block_ext_signup', array('userid' => $userid));
    foreach($exts as $ext){
        $ext->accepted = EXT_REJECTED;
        $ext->timeaccepted = time();
        $ext->acceptedby = $USER->id;
        $ext->reason = $reason;
        $DB->update_record('block_ext_signup', $ext);
        $rejectedcourses[] = $DB->get_field('course', 'fullname', array('id' => $ext->courseid)). " ($reason)";
    }
    // notify student
    $vars = array('SITE' => $SITE->shortname, 
                  'COURSELIST' => implode(",\n", $courses),
...
elseif ($action == 'acceptall'){
    $userid = required_param('userid', PARAM_INT);
    if ($exts = $DB->get_records_select('block_ext_signup', " userid = $userid AND accepted = ".EXT_PENDING)){
        foreach($exts as $ext){
            $ext->accepted = EXT_ACCEPTED;
            $ext->timeaccepted = time();
            $ext->acceptedby = $USER->id;
            $DB->update_record('block_ext_signup', $ext);
            $studentrole = $DB->get_record('role', array('shortname' => 'student'));
            $coursecontext = context_course::instance($ext->courseid);
            role_assign($studentrole->id, $userid, 0, $coursecontext->id, time(), 0, false, 'ext', time());
            $processed[] = $ext;
        }
    }
