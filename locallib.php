...
function ext_signup_print_pager($offset, $page, $maxobjects, $url){
    ...
    echo '<div class="ext-signup-pager">';
    echo implode(' - ', $pages);
    echo '</div>';
}
...
function ext_signup_print_sorted_link($label, $sorting, $sortby, $dir, $url, $return = false){
    ...
    if ($sortby == $sorting){
        if ($dir == 'DESC'){
            $str = "$label <a href=\"{$url}&amp;sortby={$sorting}&amp;dir=ASC\"><img src=\"".$OUTPUT->pix_url('t/down').'" /></a>';
        } else {
            $str = "$label <a href=\"{$url}&amp;sortby={$sorting}&amp;dir=DESC\"><img src=\"".$OUTPUT->pix_url('t/up').'" /></a>';
        }
    } else {
