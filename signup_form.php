...
class login_signup_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addHelpButton('username', 'username', 'block_ext_signup');
        ...
        $mform->addRule('email2', get_string('required'), 'required', null, 'server');
        ...
        $mform->addRule('lastname', get_string('required'), 'required', null, 'server');
        ...
        $mform->addRule('imagefile', get_string('required'), 'required', null, 'server');
        ...
        $mform->addRule('department', get_string('required'), 'required', null, 'server');
    ...
    function validation($data, $files) {
		...
		// maybe it is done by parent
		if (!empty($files)){
			$filekeys = array_keys($files);
			$uploadsize = 0;
			foreach($filekeys as $key){
				$uploadsize += @$_FILES[$key]['size'];
			}
			if ($_POST['MAX_FILE_SIZE'] < $uploadsize){
				$errors['cv'] = get_string('errortotalsize', 'block_ext_signup');
			} // size security
		}
